...
off-server, or accept regenerating after each release. An **external storage does not solve
this** — see below.
...
`filenamePattern` is excluded from the config-override filename hash — it *is* the filename.
Overriding it per transform is fine; a pattern without `{transformString}` is not, since every
transform of a source then lands on one file. See `named-transforms.md`.
...
clear-caches/all` does not touch it.
...
them from there *in addition to* the local copy.
...
**A storage is an extra place to serve from, not a replacement for the local cache.** Imager
still writes every transform to `imagerSystemPath` and still checks that local file to decide
whether the transform already exists; the upload happens afterwards. Wipe `imagerSystemPath`
and each transform is generated locally again and uploaded again. So a storage does nothing
for an ephemeral filesystem — only a transformer that does the work off-server (imgix, Gumlet,
ImageKit, Cloudflare, ImageBoss, Bunny, AWS Serverless, Craft Cloud) avoids the regeneration.
...
Two things not to do: putting credentials in the config file rather than `.env`, and switching
between `craft` and `imgixdownload` while keeping the same transform cache. Those are the two
transformers that write local files, and `transformer` is not part of the filename hash, so
already-cached transforms are served as they are instead of being regenerated by the new
transformer. Clear the cache on switch, or give each environment its own `imagerSystemPath`.
Switching to a remote transformer needs neither — it writes no local files.
