...
- **Transformer plugins** (imgix, Gumlet, ImageKit, Cloudflare Images, AWS Serverless,
  ImageBoss, Bunny.net) — offload transforms to a service. They do *not* all support the same
  features; see `references/transformers.md` before promising a parameter will work.
...
|------------------|----------------------------------------|
| Every transformer except `craft` | imgix/Gumlet/ImageKit/Cloudflare/AWS/ImageBoss/Bunny all throw `ImagerException` |
...
- **Seven settings are excluded from the config-override filename hash** — `fillTransforms`,
  `fillInterval`, `fillAttribute`, `filenamePattern`, `transformer`, `optimizeType`,
  `safeFileFormats`. Mostly uneventful, since none of them changes what a single transform
  renders. The two to know: `transformer` deliberately makes `craft` and `imgixdownload` share
  filenames, so switching between those two without clearing the cache serves the old files;
  and a `filenamePattern` without `{transformString}` collapses every transform of a source
  onto one filename. See `named-transforms.md`.
- **`transformerParams` is whitelisted on `bunny` and `cloudflareimages`.** Both prune the final
  parameter list to the params the transformer knows, so an option the service supports but the
  transformer does not list is dropped with no warning. Every other transformer forwards it
  verbatim. See `transformers.md`.
...
- **External storages do not make `craft` work on an ephemeral filesystem.** A storage is an
  extra place to serve transforms *from*; the local file in `imagerSystemPath` remains the
  cache Imager checks before transforming. Wipe that path and every transform is regenerated
  locally and re-uploaded. Only a transformer that transforms off-server avoids that.
  See `configuration.md`, `transformers.md`.
...
- "Switch this project to imgix or Gumlet" / "will `cropZoom` work on AWS Serverless?" → `transformers.md`
- "Use a `craft`-only effect but keep CDN delivery" / "transform an external image on Bunny" → `transformers.md` (Combining Transformers)
